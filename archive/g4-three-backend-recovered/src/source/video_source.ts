...
export class VideoSource extends ImageSource {
    ...
    prepare(): this {
        ...
        if (!this.texture) {
            this.texture = this.map.painter.createImageTexture(this.video);
        } else if (!this.video.paused) {
            // `Texture.update` binds and re-uploads; the hand-rolled
            // `texSubImage2D` here did the same thing with a raw context.
            this.texture.update(this.video);
        }
