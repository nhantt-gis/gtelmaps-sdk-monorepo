...
export class FillExtrusionBucket implements Bucket {
    ...
    /**
     * Whether this bucket's layer type declares a `<prefix>-pattern` property.
     *
     * `hasPattern` reads `layer.paint.get(`${prefix}-pattern`)` and calls
     * `.isConstant()` on the result. `PossiblyEvaluated.get` is a plain property
     * lookup, so a layer type without that property yields `undefined` and the
     * call throws — inside the worker, during `populate`, with a stack that
     * mentions nothing about styles. Subclasses whose layer type has no pattern
     * turn this off rather than declaring a property they will never draw.
     */
    protected get hasPatternSupport(): boolean { return true; }

    /** The style-spec prefix this bucket's pattern property is named with. */
    protected get patternPropertyPrefix(): string { return 'fill-extrusion'; }

    populate(features: Array<IndexedFeature>, options: PopulateParameters, canonical: CanonicalTileID) {
        this.features = [];
        this.hasDependencies = this.hasPatternSupport &&
            hasPattern(this.patternPropertyPrefix, this.layers, options);
        ...
        for (const {feature, id, index, sourceLayerIndex} of features) {
            ...
            if (this.hasDependencies) {
                this.features.push(addPatternDependencies(
                    this.patternPropertyPrefix, this.layers, bucketFeature, {zoom: this.zoom}, options));
            } else {
