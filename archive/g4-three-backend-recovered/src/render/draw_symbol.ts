...
/**
 * Re-places variable-anchored text for this frame's camera.
 *
 * Exported for the Three backend rather than reimplemented there. Not for the
 * arithmetic: it **writes** `bucket.text.dynamicLayoutVertexBuffer` (and the
 * icon one under `icon-text-fit`), which is state the next frame reads back. A
 * second implementation would not duplicate a calculation, it would fork a
 * buffer both backends are updating.
 */
