...
function renderHeatmapFlat(painter: Painter, layer: HeatmapStyleLayer) {
    ...
    const colorRampTexture = getHeatmapColorRampTexture(context, layer);
...
/**
 * The layer's colour ramp texture, created on first use.
 *
 * Exported for the Three backend rather than reimplemented there, and for one
 * reason that a three-line copy would not have: the texture is cached **on the
 * layer**. A second implementation would create a second texture the moment both
 * backends touched the same layer, and the symptom would be GPU memory growth
 * rather than anything visible.
 */
export function getHeatmapColorRampTexture(context: Context, layer: HeatmapStyleLayer): Texture {
