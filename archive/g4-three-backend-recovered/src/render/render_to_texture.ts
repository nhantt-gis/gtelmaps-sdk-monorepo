    ...
    renderLayer(layer: StyleLayer, renderOptions: RenderOptions): boolean {
        ...
        if (LAYERS_TO_TEXTURES[this._prevType] || (LAYERS_TO_TEXTURES[type] && isLastLayer)) {
            ...
            for (const tile of this._renderableTiles) {
                ...
                //
                // The flush is not optional. `clear` is this tier drawing — it
                // binds a framebuffer and issues `gl.clear` through MapLibre's
                // cached state — and a backend that replaced the render tier may
                // still hold the context from the previous tile's last layer,
                // with its own write masks and scissor live. A clear that lands
                // under someone else's masks does not error; it silently leaves
                // the pool object holding whatever was there before.
                painter.migratedFlush?.();
            ...
            // A backend replacing the render tier may still be holding the
            // context from the last layer drawn into the pool. `drawTerrain`
            // below is *this* tier drawing to the canvas, so it must own the
            // context first — otherwise it composites the pool textures with
            // another renderer's GL state live and its own state cache stale.
            //
            // Same shape as the four flush points in `Painter.render`: batching
            // the handover never produces wrong state, it moves *when* the state
            // is handed back, and every place this tier draws between migrated
            // layers has to say so.
            this.painter.migratedFlush?.();
            drawTerrain(this.painter, this.terrain, this._rttTiles, options);
            this._rttTiles = [];
            this.pool.freeAllObjects();
