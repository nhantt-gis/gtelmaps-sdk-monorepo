 ...
 *
 * Exported for the Three backend (`src/strict/render_three/raster_layer.ts`).
 * Reused rather than reimplemented because the caller **writes back**
 * `tile.fadeOpacity` and `parentTile.fadeOpacity` from what this returns, and
 * those values feed `tile.refreshFadeOpacity` on later frames. A second
 * implementation would not merely duplicate arithmetic; it would fork a piece of
 * animation state that MapLibre also reads.
 */
export function getFadeProperties(tile: Tile, tileManager: TileManager, fadeDuration: number, isTerrain: boolean): FadeProperties {
