...
/**
 * Exported for the Three backend (`src/strict/render_three/line_layer.ts`).
 *
 * Reused rather than reimplemented: the step-interpolant branch is a resolution
 * *heuristic* bounded by hardware limits, and a second copy of it would drift
 * from this one silently — the ramp would simply be sampled at a different
 * resolution, which no assertion catches and no screenshot attributes back.
 */
