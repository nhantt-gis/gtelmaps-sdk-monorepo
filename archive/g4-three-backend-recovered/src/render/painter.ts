...
import {Texture, type TextureImage} from './texture';
 ...
 */
export class Painter implements RenderBackend {
    context: Context;
    ...
    currentLayer: number;
    /**
     * Lets a replacement backend take over individual layers.
     *
     * Returns `true` when it drew the layer and this painter should not. A
     * backend part-way through G4 returns `false` for anything it cannot draw
     * yet — an unmigrated layer type, or a case it has not covered such as
     * patterns or terrain — and the old path runs unchanged. See `renderLayer`.
     */
    migratedLayerDraw: ((tileManager: TileManager, layer: StyleLayer, coords: Array<OverscaledTileID>, renderOptions: RenderOptions) => boolean) | null = null;
    migratedSkyDraw: ((sky: any) => boolean) | null = null;

    /**
     * Called immediately before this painter performs GL work of its own, so a
     * backend holding the context can hand it back first.
     *
     * {@link migratedLayerDraw} may leave the other renderer owning the context
     * across a run of consecutive layers — that is the point, the handover is
     * expensive. This is how the run gets closed at the right moment instead of
     * after every layer: the painter says "I am about to draw", not "a layer
     * boundary happened".
     */
    migratedFlush: (() => void) | null = null;
    ...
    _renderTileClippingMasks(layer: StyleLayer, tileIDs: Array<OverscaledTileID>, renderToTexture: boolean) {
        ...
        // Past the early return, so this is a layer boundary that genuinely
        // draws. Consecutive layers of the same source return above and leave a
        // batched handover open.
        this.migratedFlush?.();
    ...
    render(style: Style, options: PainterOptions) {
        ...
        if (this.style.sky && !(this.migratedSkyDraw && this.migratedSkyDraw(this.style.sky))) drawSky(this, this.style.sky);
        ...
        // Everything below is this painter's own drawing, after the layer loops
        // have ended — and the loops can end with a batched handover still open.
        this.migratedFlush?.();
    ...
    renderLayer(painter: Painter, tileManager: TileManager, layer: StyleLayer, coords: Array<OverscaledTileID>, renderOptions: RenderOptions) {
        ...
        // A backend replacing this render tier draws some layer types itself.
        // Handing over *here* rather than before or after the frame means the
        // other renderer draws at precisely the moment this one would have, so
        // layer order, render pass and depth all stay correct by construction
        // instead of by argument.
        //
        // The cost is a context handover per layer per pass rather than one per
        // frame — early-z and batching are lost while the migration is in
        // progress. That is the transitional cost recorded in the G4 plan, not a
        // regression to optimise away before the migration finishes.
        if (this.migratedLayerDraw && this.migratedLayerDraw(tileManager, layer, coords, renderOptions)) return;
    ...
    /**
     * Uploads an image and returns a ready-to-use texture.
     *
     * Exists so that sources do not need `painter.context.gl`. Every use of the
     * raw context they had was a GL *enum* — `gl.RGBA`, `gl.LINEAR`,
     * `gl.CLAMP_TO_EDGE` — never a GL call, so what they actually wanted was a
     * texture with standard clamped-linear filtering. Naming that removes four
     * of the six places raw WebGL escaped the render tier (G3-D).
     */
    /**
     * Attaches or detaches terrain, and owns the render-to-texture lifecycle
     * that comes with it.
     *
     * `ui/map.ts` used to construct `RenderToTexture` itself and assign it onto
     * the painter. That put a render-tier class in the hands of the UI layer, so
     * a different backend could not supply its own — it would have had to accept
     * this one. Moving both the construction and the teardown behind one call
     * takes `renderToTexture` off the backend contract entirely (G3-E).
     */
    setTerrain(terrain: Terrain | null): void {
        this.renderToTexture?.destruct();
        this.renderToTexture = terrain ? new RenderToTexture(this, terrain) : null;
    }

    createImageTexture(image: TextureImage, options?: {useMipmap?: boolean; premultiply?: boolean}): Texture {
        const gl = this.context.gl;
        const texture = new Texture(this.context, image, gl.RGBA, options);
        texture.bind(
            gl.LINEAR,
            gl.CLAMP_TO_EDGE,
            options?.useMipmap ? gl.LINEAR_MIPMAP_NEAREST : undefined
        );
        return texture;
    }
