...
import {BuildingGlassStyleLayer} from './style_layer/building_glass_style_layer';
...
export function createStyleLayer(layer: LayerSpecification | CustomLayerInterface, globalState: Record<string, any>) {
    ...
    switch (layer.type) {
        ...
        case 'building-glass':
            return new BuildingGlassStyleLayer(layer, globalState);
        case 'circle':
        ...
        default:
            // Without this the function returns `undefined` and the caller does
            // `styledLayer.setEventedParent(...)` on it — a `TypeError` whose
            // message mentions neither the layer nor the type. `addLayer`
            // validates first and never gets here; `setStyle` does not.
            throw new Error(`Layer "${(layer as {id: string}).id}" has unknown type "${(layer as {type: string}).type}".`);
    }
}
